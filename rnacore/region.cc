...
int region::smooth_join_interval_map()
{
	...
	for(JIMI it = jmap.begin(); it != jmap.end(); it++)
	{
		...
		assert(p1 >= p);
		assert(p2 > p1);
		if(p1 - p <= subregion_gap) v.push_back(PI32(p, p1));
		p = p2;
	}

	if(p < rpos && rpos - p <= subregion_gap) v.push_back(PI32(p, rpos));
...
bool region::empty_subregion(int32_t p1, int32_t p2)
{
	...
	if(p2 - p1 < subregion_length) return true;
	...
	if(ratio < subregion_overlap) return true;
