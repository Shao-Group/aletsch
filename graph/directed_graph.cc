...
{
	edge_base e(s, t);
	auto x = vv[s]->so.lower_bound(&e);

	if(x != vv[s]->so.end())
	{
		if((*x)->s == s && (*x)->t == t) return PEB(*x, true);
	}

	if(x == vv[s]->so.begin()) return PEB(null_edge, false);

	x--;

	if((*x)->s == s && (*x)->t == t) return PEB(*x, true);
	else return PEB(null_edge, false);
}
...
int directed_graph::bfs_reverse(int t, vector<int> &v, vector<int> &b, const set<int> &fb)
{
	v.clear();
	b.clear();
	if(fb.find(t) != fb.end()) return 0;
...
int directed_graph::draw(const string &file, const MIS &mis, const MES &mes, double len, bool footer)
{
	vector<int> tp;
	for(int i = 0; i < num_vertices(); i++) tp.push_back(i);
	return draw(file, mis, mes, len, tp, footer);
}

int directed_graph::draw(const string &file, const MIS &mis, const MES &mes, double len, const vector<int> &tp, bool footer)
{
	...
	for(int ii = 0; ii < tp.size(); ii++)
	{
		...
		fout.precision(1);
		fout<<fixed;
		//fout<<"\\node[mycircle, black, draw, label = below:{";
		//fout<<s.c_str() <<"}] ("<<sx<<") at ("<<pos<<" *\\len, 0.0) {"<<i<<"};\n";
		fout<<"\\node[mycircle, black, draw] ("<<sx<<") at ("<<pos<<" *\\len, 0.0) {"<<i<<"};\n";
		for(int k = 1; k < ss.size(); k++)
		{
			int d = 1 + k;
			fout<<"\\node at ($(" << sx << ".south) - (0.0, " << d << "em)$) {"<< ss[k].c_str() <<"};\n";
		}
