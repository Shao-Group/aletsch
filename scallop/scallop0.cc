...
scallop0::scallop0(const splice_graph &g, const hyper_set &h)
	: gr(g), hs(h)
{
	round = 0;
...
bool scallop0::resolve_smallest_edges(double max_ratio)
{
	int se = -1;
	int root = -1;
...
bool scallop0::resolve_negligible_edges(bool extend, double max_ratio)
{
	bool flag = false;
	//for(set<int>::iterator it = nonzeroset.begin(); it != nonzeroset.end(); it++)
	//for(int i = 1; i < gr.num_vertices() - 1; i++)
...
bool scallop0::resolve_splittable_vertex(int type, int degree, double max_ratio)
{
	int root = -1;
	double ratio = max_ratio;
...
bool scallop0::resolve_unsplittable_vertex(int type, int degree, double max_ratio)
{
	int root = -1;
	MPID pe2w;
	double ratio = max_ratio;
...
bool scallop0::resolve_hyper_edge(int fsize)
{
	edge_iterator it1, it2;
	PEEI pei;
	vector<int> v1, v2;
...
bool scallop0::resolve_trivial_vertex(int type, double jump_ratio)
{
	int root = -1;
...
bool scallop0::resolve_trivial_vertex_fast(double jump_ratio)
{
	bool flag = false;
	//for(set<int>::iterator it = nonzeroset.begin(); it != nonzeroset.end(); it++)
	vector<int> vv(nonzeroset.begin(), nonzeroset.end());
...
bool scallop0::resolve_single_trivial_vertex_fast(int i, double jump_ratio)
{
...
int scallop0::summarize_vertices()
{
	for(set<int>::iterator it = nonzeroset.begin(); it != nonzeroset.end(); it++)
	{
		int i = (*it);
...
int scallop0::add_pseudo_hyper_edges()
{
...
int scallop0::init_inner_weights()
{
	edge_iterator it1, it2;
	PEEI pei;
	for(pei = gr.edges(), it1 = pei.first, it2 = pei.second; it1 != it2; it1++)
...
int scallop0::decompose_vertex_extend(int root, MPID &pe2w)
{
...
int scallop0::decompose_vertex_replace(int root, MPID &pe2w)
{
	// reassign weights
	MID md;
	for(MPID::iterator it = pe2w.begin(); it != pe2w.end(); it++)
...
int scallop0::decompose_trivial_vertex(int x)
{
	balance_vertex(x);
...
int scallop0::classify_trivial_vertex(int x, bool fast)
{
...
int scallop0::exchange_sink(int old_sink, int new_sink)
{
	VE ve;
	edge_iterator it1, it2;
	PEEI pei;
...
int scallop0::split_merge_path(const VE &p, double wx)
{
	vector<int> v;
...
int scallop0::split_merge_path(const vector<int> &p, double ww)
{
...
int scallop0::merge_adjacent_equal_edges(int x, int y)
{
...
int scallop0::remove_edge(int e)
{
...
int scallop0::merge_adjacent_edges(int x, int y, double ww)
{
	if(i2e[x] == null_edge) return -1;
	if(i2e[y] == null_edge) return -1;
...
int scallop0::merge_adjacent_edges(int x, int y)
{
...
int scallop0::split_edge(int ei, double w)
{
...
int scallop0::balance_vertex(int v)
{
...
double scallop0::compute_balance_ratio(int v)
{
	edge_iterator it1, it2;
	PEEI pei;
	double w1 = 0, w2 = 0;
...
int scallop0::split_vertex(int x, const vector<int> &xe, const vector<int> &ye)
{
	assert(x != 0);
...
vector<int> scallop0::topological_sort()
{
	vector<PI> v;
...
int scallop0::collect_existing_st_paths()
{
...
int scallop0::collect_path(int e, int s, int t)
{
...
int scallop0::greedy_decompose()
{
	...
	while(true)
	{
		...
		collect_path(e, 0, gr.num_vertices() - 1);
		cnt++;
	}
	int n2 = paths.size();
...
int scallop0::compute_smallest_edge(int x, double &ratio)
{
	int e = -1;
	ratio = DBL_MAX;
	edge_iterator it1, it2;
...
int scallop0::draw_splice_graph(const string &file) 
{
	MIS mis;
	char buf[10240];
	for(int i = 0; i < gr.num_vertices(); i++)
