...
scallop::scallop(const splice_graph &g, const hyper_set &h1)
	: gr(g), hs(h1)
{
	round = 0;
...
scallop::scallop(const splice_graph &g, const hyper_set &h1, const hyper_set &h2)
	: gr(g), hs(h1), hx(h2)
{
	round = 0;
	if(output_tex_files == true) gr.draw(gr.gid + "." + tostring(round++) + ".tex");
